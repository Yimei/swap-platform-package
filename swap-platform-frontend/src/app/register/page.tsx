  ...
  return (
    <div className="mx-auto max-w-md py-12">
      <form onSubmit={handleSubmit} className="space-y-5 rounded-3xl border border-neutral-200 bg-white p-8 shadow-sm">
        <div>
          <p className="text-sm font-semibold text-emerald-700">建立新帳號</p>
          <h1 className="mt-2 text-3xl font-bold text-neutral-900">註冊</h1>
        </div>
        <label className="block space-y-2">
          <span className="text-sm font-medium text-neutral-700">姓名</span>
          <input className="w-full rounded-2xl border border-neutral-300 px-4 py-3" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required />
        </label>
        <label className="block space-y-2">
        ...
        <label className="block space-y-2">
          ...
          <input className="w-full rounded-2xl border border-neutral-300 px-4 py-3" type="password" minLength={8} value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} required />
        </label>
        ...
        <p className="text-center text-sm text-neutral-600">
          已經有帳號？ <Link href="/login" className="font-semibold text-emerald-700">前往登入</Link>
        </p>
      </form>
    </div>
  );
